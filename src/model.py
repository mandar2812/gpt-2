...
class HParams:
    """A simple class to manage hyperparameters."""
    ...
    def add_hparam(self, name, value):
        if name in self.__dict__:
            raise ValueError(f"Hyperparameter name '{name}' is reserved.")
    ...
    def update(self, values):
        ...
        for name, value in values.items():
            ...
            expected_type = self._hparam_types[name]
            if not isinstance(value, expected_type):
                raise ValueError(
                    f"Hyperparameter '{name}' must be of type {expected_type}."
                )
            self.__dict__[name] = value
